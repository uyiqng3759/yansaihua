...
async function getLog() {
  try {
    const { body } = await $.http.get({url: `https://hcxm.xyz/api/get-log`,timeout: 30 * 1000})
    const {log,num} = JSON.parse(body)
    console.log(`剩余可用log数量为${num}`)
    if(!num) {
      console.log(`需要等待10分钟释放log`)
      await $.wait(600 * 1000)
    }
    await updateLog(log.log)
    return log
  }catch (e) {
    await getLog()
  }
  ...
  return new Promise(async resolve => {
    ...
    if ($.isNode()) {
      Object.keys(jdCookieNode).forEach((item) => {
        if (jdCookieNode[item] && jdCookieNode[item].includes('app_open') ) {
