...
[task_local]
#秒秒币
20 7 * * * jd_ms.js, tag=秒秒币, img-url=, enabled=true

================Loon==============
[Script]
cron "20 7 * * *" script-path=jd_ms.js, tag=秒秒币

===============Surge=================
秒秒币 = type=cron,cronexp="20 7 * * *",wake-system=1,timeout=3600,script-path=jd_ms.js

============小火箭=========
秒秒币 = type=cron,script-path=jd_ms.js, cronexpr="20 7 * * *", timeout=3600, enable=true
 */
// if(+process.env.CONTAINER_NO !== 1)return
const $ = new Env('秒秒币');
...
!(async () => {
  ...
  const { body } = await $.http.get({url: `https://hcxm.xyz/api/get-signStrs`,timeout: 30 * 1000})
  const { signStrs } = JSON.parse(body)
  if(!signStrs.length)return
...
async function doTask(body) {
  ...
  body = {...body, "encryptProjectId": $.encryptProjectId, "sourceCode": sourceCode,  "ext": {},"extParam":{"businessData":{"random":`${item.random}`},"signStr":`${item.signStr}`,"sceneid":"MShPageh5"} }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.msg.includes('火爆')) {
              // await removeLog(item.log)
              console.log('wait 4mine retry task')
              // await $.wait(240 * 1000)
              await doTask(body)
            }
...
function readpacksign() {
  return new Promise(async resolve => {
    const item = await getLog() || {}
    body = `uuid=88888&clientVersion=10.3.4&client=wh5&osVersion=&area=4_48201_54794_0&networkType=unknown&functionId=signRedPackage&body={"random":${item.random},"log": "${item.signStr}","sceneid":"MShPageh5","ext":{"platform":"1","eid":"","referUrl":-1,"userAgent":-1}}&appid=SecKill2020`
    $.post(readpack(body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${err},${jsonParse(resp.body)['message']}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
async function getLog() {
  try {
    const { body } = await $.http.get({url: `https://hcxm.xyz/api/get-signStr`,timeout: 30 * 1000})
    const {signStr,num} = JSON.parse(body)
    console.log(`剩余可用signStr数量为${num}`)
    if(num === 1 || !num) {
      console.log(`需要等待10分钟释放signStr`)
      await $.wait(600 * 1000)
    }
    await updateLog(signStr?.signStr)
    return signStr
  }catch (e) {
    await getLog()
  }
...
async function updateLog(signStr) {
  try {
    return await $.http.post({url: `https://hcxm.xyz/api/update-signStr`,json: {signStr,locked: '1'},timeout: 30 * 1000})
  }catch (e) {
    await updateLog(signStr)
  }
}
