...
async function doInteractiveAssignment(projectId, encryptAssignmentId, itemId, actionType) {
  ...
  let body = { "encryptProjectId": projectId, "encryptAssignmentId": encryptAssignmentId, "sourceCode": "acexinpin0823", "itemId": itemId, "actionType": actionType, "completionFlag": "", "ext": {},"extParam":{"businessData":{"random": item.random},"signStr":item.signStr,"sceneid":"XMFhPageh5"} }
        ...
        if (err) {
          ...
          if (data) {
            ...
            if(data.msg.includes('火爆')) {
              // await removeLog(item.signStr)
              console.log('retry task')
              // await $.wait(240 * 1000)
              await doInteractiveAssignment(projectId, encryptAssignmentId, itemId, actionType)
            }
            if(data.msg.includes('风险等级未通过')) {
              $.hot = true
            }
          } else {
            console.log("没有返回数据")
