...
async function receiveFriendInvite() {
  for (let code of newShareCodes.slice(0, 1)) {
    if (code === $.farmInfo.farmUserPro.shareCode) {
...
async function initForFarm() {
  return new Promise(async resolve => {
    ...
    const h5st = await geth5st([{"key": "functionId", "value": "initForFarm"}, {
      "key": "body",
      "value": $.CryptoJS.SHA256($.toStr(body, body)).toString()
    }, {"key": "appid", "value": "wh5"}, {"key": "clientVersion", "value": "9.1.0"}], '8a2af')
    const option = {
      url: `${JD_API_HOST}?functionId=initForFarm`,
      body: `body=${encodeURIComponent(JSON.stringify(body))}&appid=wh5&clientVersion=9.1.0&h5st=${encodeURIComponent(h5st)}`,
      headers: {
        "accept": "*/*",
        "accept-encoding": "gzip, deflate, br",
...
async function request(function_id, body = {}, timeout = 2000) {
  await $.wait(timeout)
  return new Promise(async resolve => {
    $.get(await taskUrl(function_id, body), (err, resp, data) => {
      try {
        if (err) {
          console.log('\n东东农场: API查询请求失败 ‼️‼️')
          console.log(JSON.stringify(err));
...
async function taskUrl(function_id, body = {}) {
  ...
  let url = `${JD_API_HOST}?functionId=${function_id}&body=${encodeURIComponent(JSON.stringify(body))}&appid=wh5`
  ...
  switch (function_id) {
    ...
    case 'waterFriendForFarm':
      $.appId = '673a0'
      break
    case 'userMyCardForFarm':
      $.appId = '86ba5'
      break
    case 'myCardInfoForFarm':
      $.appId = '157b6'
      break
    ...
    case 'farmAssistInit':
      $.appId = '92354'
      break
    case 'clockInFollowForFarm':
      $.appId = '4a0b4'
      break
    case 'firstWaterTaskForFarm':
      $.appId = '0cf1e'
      break
    case 'waterFriendGotAwardForFarm':
      $.appId = 'd08ff'
      break
  ...
  if ($.appId) {
    ...
    const h5st = await geth5st([{"key": "functionId", "value": function_id}, {
      "key": "body",
      "value": $.CryptoJS.SHA256($.toStr(body, body)).toString()
    }, {"appid": "functionId", "value": "wh5"}], $.appId)
    url = `${url}&h5st=${encodeURIComponent(h5st)}`
  }
  return {
    url,
    headers: {
